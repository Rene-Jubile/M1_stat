...
# Calcul du sill
#sill = psill + nugget avec 
psill <- 0.6375214 
#et 
nugget <- 2.0519717
sill <- round(psill + nugget, 3)
paste0( "La valeur du sill est : ", sill)
```
...
#Précision du modèle (p) avec p = nugget / sill

p = round(nugget / sill, 3)

paste0("La précision du modèle est : ", p)
```
